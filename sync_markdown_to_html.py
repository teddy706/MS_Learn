...
# Read curriculum_content_readable.md
with open("curriculum_content_readable.md", "r", encoding="utf-8") as f:
    md_content = f.read()

# Load fluent icons & template structure from fix_unit_08_size_mismatch.py
loc = {}
with open("fix_unit_08_size_mismatch.py", "r", encoding="utf-8") as f:
    exec(f.read(), loc)
fluent_icons = loc["fluent_icons"]
master_chapters = loc["master_chapters"]

# Parse chapters and units from MD
# Pattern for Chapter: ## 🌐 01. Title
# Pattern for Unit: ### [Unit 01] Title
# - **배지(태그)**: Badge
# - **핵심 부제**: Subtitle
# #### 📋 세부 학습 내용 및 실전 프롬프트
# Body...
...
unit_blocks = re.split(r'={10,}', md_content)
parsed_units = []

for block in unit_blocks:
    block = block.strip()
    if not block or "### [Unit" not in block:
        continue
    
    # Extract Unit Num & Title
    u_match = re.search(r'###\s*\[Unit\s*(\d+)\]\s*(.+)', block)
    if not u_match:
        continue
    num_str = u_match.group(1).strip()
    title_str = u_match.group(2).strip()
    
    # Extract Badge
    badge_match = re.search(r'-\s*\*\*배지\(태그\)\*\*:\s*`?([^`\n]+)`?', block)
    badge_str = badge_match.group(1).strip() if badge_match else "COPILOT"
    
    # Extract Subtitle
    sub_match = re.search(r'-\s*\*\*핵심 부제\*\*:\s*(.+)', block)
    subtitle_str = sub_match.group(1).strip() if sub_match else ""
    
    # Extract Body content after #### 📋 세부 학습 내용 및 실전 프롬프트
    body_parts = block.split("#### 📋 세부 학습 내용 및 실전 프롬프트")
    body_text = body_parts[1].strip() if len(body_parts) > 1 else ""
...
print(f"Parsed {len(parsed_units)} units from markdown.")

# Convert raw body text into rich HTML cards/boxes
def format_unit_body_html(unit_idx, unit_data, original_body_html):
    # If it's Unit 08 (Architecture diagram) or Unit 26 (Mermaid Topology), preserve the specialized diagram format
    if "보안을 포기하지 않고 최고의 AI를 사용한다" in unit_data["title"] or unit_data["num"] == "08":
        return original_body_html
    if "Mermaid" in unit_data["title"] or "graph LR" in unit_data["body_raw"]:
        return f"""
...
"""
    # Otherwise, use existing enhanced HTML structure if available, or generate from raw markdown text
    if original_body_html and len(original_body_html.strip()) > 50:
        return original_body_html
    
    # Fallback formatting
    lines = unit_data["body_raw"].split("\n")
    html_lines = ["<div class='space-y-3 text-left my-2 text-base'>"]
    for l in lines:
        l = l.strip()
        if not l:
            continue
        if l.startswith(">"):
            prompt_txt = l.lstrip(">").replace("💬 **[실전 Copilot 프롬프트]**", "").strip()
            if prompt_txt:
                html_lines.append(f"""<blockquote><p class="font-mono font-semibold text-indigo-950">{prompt_txt}</p></blockquote>""")
        elif l.startswith("•"):
            html_lines.append(f"""<div class="p-2.5 bg-white rounded-xl border border-slate-200 shadow-2xs text-slate-800">{l}</div>""")
        elif l.startswith("#####"):
            html_lines.append(f"""<h4 class="font-black text-slate-900 text-lg mt-2">{l.replace('#####', '').strip()}</h4>""")
        else:
            html_lines.append(f"""<p class="text-slate-700 leading-relaxed">{l}</p>""")
    html_lines.append("</div>")
    return "\n".join(html_lines)

# Map parsed markdown units to master chapters
unit_idx_counter = 0
for chap in master_chapters:
    for u in chap["units"]:
        if unit_idx_counter < len(parsed_units):
            p_unit = parsed_units[unit_idx_counter]
            u["title"] = p_unit["title"]
            u["badge"] = p_unit["badge"]
            u["subtitle"] = p_unit["subtitle"]
            # Body updated with formatted HTML
            u["body"] = format_unit_body_html(unit_idx_counter, p_unit, u.get("body", ""))
            unit_idx_counter += 1
...
print("Successfully synchronized all markdown edits directly into AX_CA_Edu_GHLEE.html!")

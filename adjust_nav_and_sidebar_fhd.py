...
with open("optimize_for_fhd_1080p.py", "r", encoding="utf-8") as f:
    code = f.read()
...
<body class="h-full flex flex-col antialiased">
    ...
    <!-- Main Container: 1/3 Sidebar + 2/3 Content Stage -->
    <div class="flex-1 flex overflow-hidden relative">

        <!-- Left Journey Sidebar (Occupying ~1/3 of the Screen, w-full lg:w-[32%] xl:w-[33.33%] max-w-[620px]) -->
        <aside id="sidebar" class="no-print w-full lg:w-[32%] xl:w-[33.33%] max-w-[620px] min-w-[340px] bg-white/95 backdrop-blur-md border-r border-slate-200 flex flex-col shrink-0 z-30 shadow-xs">
            ...
            <div id="sidebarAppBanner" class="p-4 md:p-5 bg-slate-50 border-b border-slate-200 flex items-center justify-between transition-colors">
                <div class="flex items-center space-x-3.5 min-w-0">
                    <span id="activeAppIcon" class="w-9 h-9 flex items-center justify-center shrink-0">{fluent_icons["copilot"]}</span>
                    <div class="min-w-0 flex-1">
                        <div id="activeAppName" class="font-black text-sm md:text-base text-slate-900 leading-tight truncate">Work IQ & Copilot Core</div>
                        <div id="activePartNum" class="text-xs md:text-sm text-indigo-700 font-bold mt-0.5 break-keep">01. M365 Copilot의 변화, 일을 더 잘 이해하게 된 AI</div>
                    </div>
                </div>
            ...
            <!-- Slide List Scroll Area (1/3 Wide Screen: Full Titles & Rich Badges Visible) -->
            <div class="flex-1 overflow-y-auto p-3 space-y-2" id="slideListNav">
                ...
                <div class="slide-nav-item p-3.5 rounded-2xl cursor-pointer transition-all text-left flex items-start space-x-3 hover:bg-slate-50 border border-transparent" id="nav-item-{idx}" onclick="goToSlide({idx})">
                    <span class="shrink-0 w-8 h-8 rounded-xl bg-slate-100 text-slate-700 flex items-center justify-center font-mono text-xs md:text-sm font-black mt-0.5 item-num-badge">{s["num"]}</span>
                    <div class="flex-1 min-w-0">
                        <div class="text-sm md:text-base font-bold text-slate-800 break-keep leading-snug item-title flex items-start space-x-2">
                            <span class="scale-75 shrink-0 mt-0.5">{s["app_icon_svg"]}</span>
                            <span>{s["title"]}</span>
                        </div>
                        <div class="text-xs text-slate-500 mt-1 font-semibold break-keep">{s["subtitle"]}</div>
                    </div>
                </div>
        ...
        <!-- Center Workspace: 2/3 Main Stage -->
        <main class="flex-1 flex flex-col overflow-hidden relative">
            ...
            <div id="appThemeHeader" class="no-print h-11 bg-white/80 backdrop-blur-sm border-b border-slate-200 text-slate-700 px-6 flex items-center justify-between transition-all duration-500 shrink-0">
                <div class="flex items-center space-x-2.5 text-xs md:text-sm font-semibold truncate">
                    ...
                    <span id="bannerSlideTitle" class="truncate max-w-2xl text-slate-900 font-extrabold break-keep text-xs md:text-sm">웹 기반 범용 AI vs M365 Copilot: 기업 업무에 최적화된 차이점</span>
                </div>
            ...
            <!-- Single Card View Container (2/3 Proportion Stage) -->
        ...
        function renderSlide(index) {{
            ...
            document.querySelectorAll('.slide-nav-item').forEach((item, i) => {{
                if (i === index) {{
                    ...
                    item.querySelector('.item-num-badge').className = `shrink-0 w-8 h-8 rounded-xl bg-slate-900 text-white flex items-center justify-center font-mono text-xs md:text-sm font-black mt-0.5 item-num-badge`;
                    item.querySelector('.item-title').className = `text-sm md:text-base font-black text-slate-900 break-keep leading-snug item-title flex items-start space-x-2`;
                    ...
                    item.querySelector('.item-num-badge').className = `shrink-0 w-8 h-8 rounded-xl bg-slate-100 text-slate-600 flex items-center justify-center font-mono text-xs md:text-sm font-bold mt-0.5 item-num-badge`;
                    item.querySelector('.item-title').className = `text-sm md:text-base font-bold text-slate-700 break-keep leading-snug item-title flex items-start space-x-2`;
                }}
            }});
...
print("Successfully applied 1/3 sidebar width and compact refined navigation fonts!")

...
- ✓ 기존 문서 권한 자동 준수: 내가 읽기 권한을 가진 문서에 한해서만 안전 답변 
 ...
 핵심 통찰: 웹 AI가 세상의 일반 지식을 아는 도우미라면, M365 Copilot은 기존 문서 권한과 테넌트 보안을 체계적으로 준수하는 전담 동료입니다.
...
##### Work IQ가 실무를 이해하는 방식
 ...
 💡 참고: 좋은 프롬프트 작성 원칙(GCSE)은 Unit 15에서 자세히 다룹니다.
...
##### 대용량 로그 & 수치 복합 추론
 ...
 💡 실무 팁: 멀티모달 비전 분석은 1차 스크리닝 및 권고안 초안 생성용으로 활용하며, 실제 망 구성 변경 전 최종 확인은 엔지니어가 수행합니다. (좋은 프롬프트 작성 원칙은 Unit 15에서 자세히 다룹니다)
...
✨ Copilot Chat Cross-App Query 
 ...
 💡 실무 UI 팁: 채팅창에 ' / '를 입력하면 나타나는 팝업 피커에서 Teams 채널, SharePoint 문서, 메일을 직접 선택하여 프롬프트에 첨부합니다. (좋은 프롬프트 작성 원칙은 Unit 15에서 자세히 다룹니다)
...
##### Your Microsoft 365 Tenant
 ...
 🛡️ 고객 데이터는 테넌트 내에서 안전하게 보호되며, AI 모델 재학습에 사용되지 않습니다.
...
- **핵심 부제**: 웹 검색 인용, SharePoint 사내 문서, 페더레이션 커넥터(Canva, HubSpot, FactSet), Power BI 대시보드
...
- • Edit with Copilot: 셀/수식 직접 편집 (구 Agent Mode)
...
⚡ Z-Score Anomaly Detection 
 ...
 "백홀 트래픽 로그에서 Z-Score가 +2.5 이상인 이상 구간을 빨간색 조건부 서식으로 강조하고, Inbound/Outbound 대역폭 급증 구간과의 상관관계를 분석해줘."
...
##### 상관계수 히트맵 시각화
 ...
 "PRB 사용률, 드롭률, 다운로드 처리량(Throughput) 간의 상관관계를 파이썬 seaborn heatmap으로 시각화하여 현재 시트의 G2 셀에 삽입해줘."
...
### [Unit 29] Word Copilot 엔지니어링 3대 핵심 비법
- **분류 태그**: `WORD • 5 MAGIC SECRETS` | **솔루션**: `Microsoft Word`
- **핵심 부제**: KT 통신 표준 문서, 긴급 점검 보고서, 제안서 작성을 위한 3가지 실무 작성 비법
...
1. 초안 생성 (Draft) 
 ...
 ✨ 
 "/files KT_5G_수도권_기지국_품질지표_2026.xlsx 를 기반으로 통신망 긴급 점검 SOP 초안을 표 형식으로 작성해줘." 
...
- **핵심 부제**: 긴 문서 핵심 내용 요약, 특정 내용에 대한 심층 질의 및 해결 방안 도출
...
📑 CAPEX / OPEX Investment Proposal 
 ...
 "/files 'KT_5G_설비투자_CAPEX_예산안_2026.csv'를 근거로 코어망 현대화·무선망 증설·전송망 고도화·지능형 관제 4대 투자 항목의 노후 설비 교체 경영진 제출용 설비투자 기안서 초안을 작성해줘. 항목별 초기 투자비용(CAPEX), 연간 OPEX 절감액, MTTR 개선율을 근거로 3년 누적 절감액과 투자 회수기간(ROI)을 계산해줘. 파일에 없는 수치는 임의로 생성하지 말아줘. 투자비용, 연간 절감액, 3년 누적 절감액, 회수기간을 표로 정리하고, 경영진이 투자 필요성을 빠르게 판단할 수 있도록 핵심 효과와 투자 근거를 요약해줘."
...
Step 1 
 ...
 ✨ 
 "KT_2026_5G망_현대화_기술보고서.docx를 기반으로 KT 임원 의사결정을 위한 4장의 핵심 슬라이드 초안을 작성해줘. 각 슬라이드는 핵심 메시지, 주요 근거 데이터, 시각화 아이디어, 의사결정 포인트를 포함해줘. 보고서에 없는 수치나 사실은 임의로 생성하지 말고 ‘확인 필요’로 표시해줘." 
...
- • 메타데이터: 출처 워터마크
 ...
 "/files 'KT_2026_5G망_현대화_기술보고서.docx' 파일로부터 프레젠테이션을 생성해줘. 임원 보고에 적합하도록 장황한 글을 줄이고, 핵심 성과 지표(KPI)와 타임라인을 시각적 카드로 구성해줘."
...
- **핵심 부제**: KT 기지국 관제, 백본 트래픽 분석, 장애 복구 SOP, 해외 TAC 메일 작성을 위한 4대 실무 치트시트

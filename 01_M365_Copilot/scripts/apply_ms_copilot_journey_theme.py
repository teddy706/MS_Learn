...
# Update Slide 04 with user's uploaded M365 architecture layout
slide_04_body = """
<div class="p-6 md:p-8 bg-white/95 rounded-3xl border border-slate-200 shadow-sm max-w-5xl mx-auto my-2 text-left">
    <div class="grid grid-cols-12 gap-6 items-start">
        ...
        <div class="col-span-12 md:col-span-8 p-6 bg-gradient-to-br from-slate-50/80 via-indigo-50/40 to-pink-50/40 rounded-3xl border-2 border-indigo-200 shadow-md">
            ...
            <div class="space-y-4">
                ...
                <div class="p-5 bg-white rounded-2xl border border-slate-200 shadow-sm">
                    <div class="flex justify-between items-center mb-3">
                        ...
                        <span class="px-2.5 py-0.5 bg-indigo-100 text-indigo-800 text-[11px] font-bold rounded-full">Encrypted Tenant Boundary</span>
                    </div>
                    ...
                    <div class="p-4 bg-slate-50 rounded-xl border border-slate-200">
                        ...
                        <div class="text-[11px] text-slate-500 mb-3">Files, mailboxes, chat data, videos, etc.</div>

                        <div class="grid grid-cols-2 gap-3">
                            <div class="p-3 bg-white rounded-xl border border-slate-200 flex items-center space-x-3">
                                <span class="w-8 h-8 rounded-lg bg-sky-500 text-white flex items-center justify-center text-xs font-bold shadow-xs">E</span>
                                <div>
                            ...
                            <div class="p-3 bg-white rounded-xl border border-slate-200 flex items-center space-x-3">
                                <span class="w-8 h-8 rounded-lg bg-blue-500 text-white flex items-center justify-center text-xs font-bold shadow-xs">☁️</span>
                                <div>
                            ...
                            <div class="p-3 bg-white rounded-xl border border-slate-200 flex items-center space-x-3">
                                <span class="w-8 h-8 rounded-lg bg-indigo-600 text-white flex items-center justify-center text-xs font-bold shadow-xs">T</span>
                                <div>
                            ...
                            <div class="p-3 bg-white rounded-xl border border-slate-200 flex items-center space-x-3">
                                <span class="w-8 h-8 rounded-lg bg-teal-600 text-white flex items-center justify-center text-xs font-bold shadow-xs">S</span>
                                <div>
                ...
                <div class="grid grid-cols-2 gap-4">
                    ...
                    <div class="p-4 bg-gradient-to-r from-blue-600 via-purple-600 to-pink-500 text-white rounded-2xl shadow-md flex items-center space-x-3">
                        ...
                        <div>
                            <div class="text-[11px] font-extrabold leading-tight uppercase opacity-90">Microsoft</div>
                    ...
                    <div class="p-4 bg-white rounded-2xl border border-slate-200 shadow-sm text-center">
                        ...
                        <div class="text-[11px] text-slate-500 mb-1">subscription for Microsoft 365</div>
                        <div class="text-xs font-bold text-indigo-600">Large Language Model (LLM)</div>
                    </div>
                </div>
...
journey_html = f"""<!DOCTYPE html>
<html lang="ko" class="h-full" id="htmlRoot">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>30-Day AI Productivity Journey | Microsoft 365 Copilot 통신·네트워크 실무 마스터</title>
        ...
        tailwind.config = {{
            ...
            theme: {{
                extend: {{
                    ...
                    fontFamily: {{
                        sans: ['Segoe UI', 'Pretendard', '-apple-system', 'BlinkMacSystemFont', 'Roboto', 'sans-serif'],
    ...
    <style>
        ...
        * {{
        ...
        /* Microsoft 365 Official Hero Clean Light Background */
        body {{
        ...
        /* Microsoft Official Prompt Card Styles */
        .ms-hero-card {{
            background: #ffffff;
        ...
        /* Blockquote Prompt Style (MS Learn Copilot Style) */
        blockquote {{
...
<body class="h-full flex flex-col antialiased">
    ...
    <!-- Top Global Microsoft Header (Clean MS Light Style) -->
    <header class="no-print h-14 bg-white/90 backdrop-blur-md border-b border-slate-200 text-slate-800 flex items-center justify-between px-4 z-40 shrink-0 shadow-xs">
        <div class="flex items-center space-x-3">
            ...
            <div class="flex items-center space-x-2.5">
                ...
                <span class="text-xs text-slate-500 font-semibold hidden sm:inline uppercase tracking-wider">The Great Copilot Journey (33 Units)</span>
            </div>
        </div>
        ...
        <div class="flex items-center space-x-2.5">
            ...
            <div class="flex bg-slate-100 p-0.5 rounded-full border border-slate-200 text-xs font-bold">
                ...
                <button id="portalModeBtn" onclick="setViewMode('portal')" class="px-3 py-1 rounded-full text-slate-600 hover:text-slate-900 transition-all flex items-center space-x-1">
                    <span>📑</span>
                    <span class="hidden md:inline">포털 가이드</span>
                </button>
            </div>
    ...
    <div class="flex-1 flex overflow-hidden relative">
        ...
        <main class="flex-1 flex flex-col overflow-hidden relative">
            ...
            <!-- Slide View Container (Presentation Stage) -->
            <div id="slideViewStage" class="flex-1 overflow-y-auto p-4 md:p-8 flex items-center justify-center">
                <div id="activeSlideCard" class="w-full max-w-6xl ms-hero-card p-8 md:p-12 transition-all duration-300 text-center flex flex-col justify-between min-h-[620px]">
                    <!-- Dynamic Slide Content injected by JS -->
                </div>
            </div>

            <!-- Scroll Portal Mode Container (All 33 Units) -->
            <div id="portalViewStage" class="flex-1 overflow-y-auto p-6 md:p-12 space-y-10 hidden">
                {"".join([f'''
                <article id="portal-slide-{idx}" class="max-w-6xl mx-auto ms-hero-card p-8 md:p-14 slide-page">
        ...
        function renderSlide(index) {{
            ...
            card.innerHTML = `
                <div>
                    ...
                    <div class="flex items-center justify-between mb-4">
                        ...
                        <div class="flex items-center space-x-3">
                            ...
                            <button onclick="copyPromptContent(${{index}})" class="px-3.5 py-1.5 bg-slate-900 hover:bg-indigo-600 text-white rounded-full text-xs font-bold transition-all flex items-center space-x-1.5 shadow-sm" title="프롬프트 복사">
                                <span>📋</span>
                                <span>프롬프트 복사</span>
                            </button>
                        </div>
                    ...
                    <p class="text-base md:text-lg text-slate-600 font-medium mb-6 max-w-4xl mx-auto">${{slide.subtitle}}</p>
                </div>
                ...
                <div class="mt-6 pt-4 border-t border-slate-100 flex items-center justify-between text-xs md:text-sm text-slate-500 font-medium">
                    ...
                    <div class="flex items-center space-x-2.5">
                        <button onclick="prevSlide()" class="px-4 py-2 bg-slate-100 hover:bg-slate-200 text-slate-800 font-bold rounded-full transition-colors text-xs md:text-sm">◀ 이전</button>
                        <button onclick="nextSlide()" class="px-5 py-2 bg-slate-900 hover:bg-indigo-600 text-white font-bold rounded-full shadow-xs transition-colors text-xs md:text-sm">다음 ▶</button>
                    </div>
                </div>
            `;
...
print(f"Successfully applied official Learn Copilot Today design theme at {output_path}")

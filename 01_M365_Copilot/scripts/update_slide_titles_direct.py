...
# Load master chapters from fix_unit_08_size_mismatch.py
loc = {}
...
# Update title of each unit in master_chapters
unit_idx = 1
for chap in master_chapters:
    for u in chap["units"]:
        num_str = f"{unit_idx:02d}"
        if num_str in new_titles_map:
            u["title"] = new_titles_map[num_str]
        unit_idx += 1
...
with open("/Users/gwanghee/Documents/110_Github/MS_Learn/curriculum_content_readable.md", "w", encoding="utf-8") as f:
    f.write("\n".join(md_lines))

print("Direct title update completely succeeded across all files!")

...
# Slide 04 layout
slide_04_body = """
<div class="p-4 md:p-5 bg-white/95 rounded-2xl border border-slate-200 shadow-sm w-full mx-auto text-left">
    <div class="grid grid-cols-12 gap-4 items-start">
        
        <!-- Left Column: Users & Apps -->
        <div class="col-span-12 md:col-span-4 space-y-3">
            <div class="p-3 bg-slate-50 rounded-xl border border-slate-200">
                <div class="text-[11px] font-bold text-slate-500 uppercase tracking-wider mb-1.5">Your users and devices</div>
                <div class="flex items-center space-x-2.5 p-2 bg-white rounded-lg shadow-xs border border-slate-200">
                    ...
                    <span class="text-xs font-semibold text-slate-700">사내 인증 엔지니어</span>
                </div>
            </div>

            <div class="p-3 bg-slate-50 rounded-xl border border-slate-200">
                <div class="text-[11px] font-bold text-slate-500 uppercase tracking-wider mb-1.5">Apps on your devices</div>
                <div class="grid grid-cols-2 gap-1.5 p-1.5 bg-white rounded-lg shadow-xs border border-slate-200">
                    <div class="flex items-center space-x-1.5 p-1 bg-sky-50 rounded text-xs font-bold text-sky-800"><span>🌊</span> Edge</div>
                    <div class="flex items-center space-x-1.5 p-1 bg-blue-50 rounded text-xs font-bold text-blue-800"><span>📝</span> Word</div>
                    <div class="flex items-center space-x-1.5 p-1 bg-indigo-50 rounded text-xs font-bold text-indigo-800"><span>💬</span> Teams</div>
                    <div class="flex items-center space-x-1.5 p-1 bg-emerald-50 rounded text-xs font-bold text-emerald-800"><span>📊</span> Excel</div>
                    <div class="flex items-center space-x-1.5 p-1 bg-blue-50 rounded text-xs font-bold text-blue-900"><span>✉️</span> Outlook</div>
                    <div class="flex items-center space-x-1.5 p-1 bg-purple-50 rounded text-xs font-bold text-purple-800"><span>➕</span> More</div>
                </div>
            </div>
        </div>

        <!-- Center & Right: Microsoft 365 service boundary Box -->
        <div class="col-span-12 md:col-span-8 p-4 bg-gradient-to-br from-slate-50/80 via-indigo-50/40 to-pink-50/40 rounded-2xl border-2 border-indigo-200 shadow-md">
            <div class="space-y-2.5">
                <div class="p-3.5 bg-white rounded-xl border border-slate-200 shadow-sm">
                    <div class="flex justify-between items-center mb-1.5">
                        <h3 class="text-xs md:text-sm font-black text-slate-900">Your Microsoft 365 tenant</h3>
                        <span class="px-2 py-0.5 bg-indigo-100 text-indigo-800 text-[10px] font-bold rounded-full">Encrypted Tenant</span>
                    </div>
                    
                    <div class="p-1.5 bg-gradient-to-r from-blue-50 to-indigo-50 border border-blue-200 rounded-lg text-center font-bold text-xs text-indigo-950 mb-2">
                        Microsoft Graph (Work IQ + Entra ID ACL)
                    </div>

                    <div class="p-2.5 bg-slate-50 rounded-lg border border-slate-200">
                        <div class="text-[11px] font-bold text-slate-800 mb-1.5">Your customer data (Files, mailboxes, chats)</div>
                        <div class="grid grid-cols-2 gap-1.5 text-xs">
                            <div class="p-1.5 bg-white rounded border border-slate-200 flex items-center space-x-2">
                                <span class="w-5 h-5 rounded bg-sky-500 text-white flex items-center justify-center text-[10px] font-bold">E</span>
                                <div class="font-bold text-slate-800 text-[11px]">Exchange mail</div>
                            </div>
                            <div class="p-1.5 bg-white rounded border border-slate-200 flex items-center space-x-2">
                                <span class="w-5 h-5 rounded bg-blue-500 text-white flex items-center justify-center text-[10px] font-bold">☁️</span>
                                <div class="font-bold text-slate-800 text-[11px]">OneDrive files</div>
                            </div>
                            <div class="p-1.5 bg-white rounded border border-slate-200 flex items-center space-x-2">
                                <span class="w-5 h-5 rounded bg-indigo-600 text-white flex items-center justify-center text-[10px] font-bold">T</span>
                                <div class="font-bold text-slate-800 text-[11px]">Teams chats</div>
                            </div>
                            <div class="p-1.5 bg-white rounded border border-slate-200 flex items-center space-x-2">
                                <span class="w-5 h-5 rounded bg-teal-600 text-white flex items-center justify-center text-[10px] font-bold">S</span>
                                <div class="font-bold text-slate-800 text-[11px]">SharePoint docs</div>
                            </div>
                        </div>
                    </div>
                </div>
                ...
                <div class="grid grid-cols-2 gap-2.5">
                    <div class="p-2.5 bg-gradient-to-r from-blue-600 via-purple-600 to-pink-500 text-white rounded-xl shadow-md flex items-center space-x-2">
                        <div class="w-7 h-7 rounded-lg bg-white/20 flex items-center justify-center text-sm">✨</div>
                        <div>
                            <div class="text-[9px] font-extrabold leading-tight uppercase opacity-90">Microsoft</div>
                            <div class="text-xs font-black">365 Copilot</div>
                        </div>
                    </div>
                    <div class="p-2 bg-white rounded-xl border border-slate-200 shadow-sm text-center">
                        <div class="font-bold text-slate-900 text-xs">Azure OpenAI (GPT-5.6 / Sonnet 5)</div>
                        <div class="text-[10px] text-slate-500">Zero-Data Retention</div>
                    </div>
                </div>
            </div>

            <div class="mt-2.5 text-center">
                <span class="inline-block px-3 py-1 bg-white text-slate-900 font-extrabold text-[11px] rounded-full shadow-xs border border-indigo-200">
                    🛡️ Microsoft 365 service boundary (격리된 보안 경계)
                </span>
            </div>
        </div>
...
collapsible_html = f"""<!DOCTYPE html>
...
<head>
    ...
    <style>
        ...
        * {{
        ...
        /* Adaptive 16:9 Presentation Stage Wrapper */
        #slideStageWrapper {{
            display: flex;
            align-items: center;
        ...
        /* The 16:9 Scalable Slide Canvas */
        .ms-16-9-canvas {{
            width: 1200px;
            height: 675px;
            max-width: 100%;
            ...
            border-radius: 28px;
            box-shadow: 0 16px 45px -8px rgba(15, 23, 42, 0.08), 0 4px 18px -2px rgba(15, 23, 42, 0.04);
            display: flex;
        ...
        @media print {{
            ...
            .slide-page {{ page-break-after: always; break-after: page; width: 100% !important; height: auto !important; transform: none !important; }}
...
<body class="h-full flex flex-col antialiased">
    ...
    <header class="no-print h-14 bg-white/90 backdrop-blur-md border-b border-slate-200 text-slate-800 flex items-center justify-between px-4 z-40 shrink-0 shadow-xs">
        <div class="flex items-center space-x-3">
            ...
            <button id="sidebarToggleBtn" onclick="toggleSidebar()" class="w-9 h-9 flex items-center justify-center rounded-xl hover:bg-slate-100 transition-colors text-slate-700 font-bold" title="사이드바 네비게이션 접기/펼치기 (단축키: B)">
            ...
            <div class="flex items-center space-x-2.5">
                ...
                <span class="text-xs text-slate-500 font-bold hidden sm:inline uppercase tracking-wider">통신·네트워크 실무 마스터 (33 덱)</span>
            </div>
        </div>
    ...
    <div class="flex-1 flex overflow-hidden relative">
        ...
        <aside id="sidebar" class="no-print w-80 bg-white/95 backdrop-blur-md border-r border-slate-200 flex flex-col shrink-0 z-30 shadow-xs">
            <!-- Sidebar Header: Active App Info & Collapse Button -->
        ...
        <main class="flex-1 flex flex-col overflow-hidden relative">
            ...
            <!-- Slide View Container: Responsive Scaling Stage -->
            <div id="slideViewStage" class="flex-1 overflow-hidden relative">
                <div id="slideStageWrapper">
                    <div id="activeSlideCard" class="ms-16-9-canvas">
                        <!-- Dynamic Slide Content injected by JS -->
                    </div>
                </div>
            </div>

            <!-- Scroll Portal Mode Container (All 33 Units) -->
            <div id="portalViewStage" class="flex-1 overflow-y-auto p-6 md:p-12 space-y-12 hidden">
                {"".join([f'''
                <article id="portal-slide-{idx}" class="max-w-5xl mx-auto bg-white border border-slate-200 rounded-3xl p-8 md:p-12 shadow-sm slide-page">
                    <div>
    ...
    <script>
        ...
        function toggleSidebar() {{
            ...
            if (sidebarCollapsed) {{
                ...
                showToast('사이드바가 축소되었습니다. (단축키: B 로 복원)');
            }} else {{
                sidebar.classList.remove('collapsed');
                toggleIcon.textContent = '☰';
            }}
            
            // Re-calculate auto scale smoothly after transition
            setTimeout(autoScaleSlide, 320);
        }}

        // Auto-Scale 16:9 Canvas to fit browser window seamlessly
        function autoScaleSlide() {{
            if (viewMode !== 'slide') return;
            const wrapper = document.getElementById('slideStageWrapper');
            const canvas = document.getElementById('activeSlideCard');
            if (!wrapper || !canvas) return;

            const availWidth = wrapper.clientWidth - 32;
            const availHeight = wrapper.clientHeight - 32;
            ...
            // Cap scale between 0.45 and 1.25 for optimal readability
        ...
        function renderSlide(index) {{
            ...
            card.innerHTML = `
                <div>
                    ...
                    <h1 class="text-3xl font-black text-slate-900 mb-2 leading-tight tracking-tight break-keep max-w-4xl mx-auto">${{slide.title}}</h1>
                    <p class="text-sm md:text-base text-slate-600 font-medium mb-3 max-w-3xl mx-auto break-keep">${{slide.subtitle}}</p>
                </div>
                ...
                <div class="mt-3 pt-3 border-t border-slate-100 flex items-center justify-between text-xs text-slate-500 font-medium">
                    ...
                    <div class="flex items-center space-x-2">
                        <button onclick="prevSlide()" class="px-4 py-1.5 bg-slate-100 hover:bg-slate-200 text-slate-800 font-bold rounded-full transition-colors">◀ 이전</button>
        ...
        // Window resize event for seamless scaling
...
with open(output_path, "w", encoding="utf-8") as f:
    f.write(collapsible_html)

with open(index_path, "w", encoding="utf-8") as f:
    f.write(collapsible_html)

print(f"Successfully added collapsible sidebar navigation to {output_path} and {index_path}")

...
with open("adjust_nav_and_sidebar_fhd.py", "r", encoding="utf-8") as f:
    code = f.read()
    ...
    <style>
        * {{
        ...
        .fhd-card-stage {{
            ...
            border-radius: 28px;
            box-shadow: 0 14px 36px -8px rgba(15, 23, 42, 0.07), 0 4px 16px -2px rgba(15, 23, 42, 0.03);
...
<body class="h-full flex flex-col antialiased">
    ...
    <!-- Main Container: 2/5 Sidebar (40%) + 3/5 Content Stage (60%) -->
    <div class="flex-1 flex overflow-hidden relative">

        <!-- Left Journey Sidebar (Occupying 2/5 (40%) of the Screen, with same font size as Navigation Bar) -->
        <aside id="sidebar" class="no-print w-full lg:w-[40%] xl:w-[40%] max-w-[760px] min-w-[360px] bg-white/95 backdrop-blur-md border-r border-slate-200 flex flex-col shrink-0 z-30 shadow-xs">
            ...
            <div id="sidebarAppBanner" class="p-3.5 md:p-4 bg-slate-50 border-b border-slate-200 flex items-center justify-between transition-colors">
                <div class="flex items-center space-x-3 min-w-0">
                    <span id="activeAppIcon" class="w-8 h-8 flex items-center justify-center shrink-0">{fluent_icons["copilot"]}</span>
                    <div class="min-w-0 flex-1">
                        ...
                        <div id="activePartNum" class="text-xs text-indigo-700 font-bold mt-0.5 break-keep">01. M365 Copilot의 변화, 일을 더 잘 이해하게 된 AI</div>
                    </div>
                </div>
                <div class="flex items-center space-x-1.5 shrink-0 ml-3">
                    <span id="slideCounterBadge" class="text-xs font-mono font-bold bg-white px-2.5 py-0.5 rounded-full border border-slate-200 text-slate-700 shadow-2xs">
                        01 / {total_units:02d}
                    </span>
            ...
            <!-- Slide List Scroll Area (2/5 Screen: Nav-Bar Equivalent Text Size) -->
            <div class="flex-1 overflow-y-auto p-3 space-y-1.5" id="slideListNav">
                {"".join([f'''
                <div class="slide-nav-item p-3 rounded-2xl cursor-pointer transition-all text-left flex items-start space-x-3 hover:bg-slate-50 border border-transparent" id="nav-item-{idx}" onclick="goToSlide({idx})">
                    <span class="shrink-0 w-7 h-7 rounded-xl bg-slate-100 text-slate-700 flex items-center justify-center font-mono text-xs font-bold mt-0.5 item-num-badge">{s["num"]}</span>
                    <div class="flex-1 min-w-0">
                        <div class="text-xs md:text-sm font-semibold text-slate-800 break-keep leading-snug item-title flex items-start space-x-2">
                            <span class="scale-75 shrink-0 mt-0.5">{s["app_icon_svg"]}</span>
                            <span>{s["title"]}</span>
                        </div>
                        <div class="text-2xs md:text-xs text-slate-500 mt-1 font-medium break-keep">{s["subtitle"]}</div>
                    </div>
                </div>
                ''' for idx, s in enumerate(cleaned_slides)])}
            </div>

            <!-- Sidebar Footer (Compact Nav-Equivalent Size) -->
        ...
        <!-- Center Workspace: 3/5 Main Stage (60%) -->
        <main class="flex-1 flex flex-col overflow-hidden relative">
            ...
            <!-- Single Card View Container (3/5 Proportion Stage) -->
            ...
            <div id="portalViewStage" class="flex-1 overflow-y-auto p-6 md:p-10 space-y-10 hidden">
                {"".join([f'''
                <article id="portal-slide-{idx}" class="max-w-4xl mx-auto fhd-card-stage p-8 md:p-10 slide-page">
                    <div>
        ...
        function renderSlide(index) {{
            ...
            document.querySelectorAll('.slide-nav-item').forEach((item, i) => {{
                if (i === index) {{
                    ...
                    item.querySelector('.item-num-badge').className = `shrink-0 w-7 h-7 rounded-xl bg-slate-900 text-white flex items-center justify-center font-mono text-xs font-bold mt-0.5 item-num-badge`;
                    item.querySelector('.item-title').className = `text-xs md:text-sm font-bold text-slate-900 break-keep leading-snug item-title flex items-start space-x-2`;
                    ...
                    item.querySelector('.item-num-badge').className = `shrink-0 w-7 h-7 rounded-xl bg-slate-100 text-slate-600 flex items-center justify-center font-mono text-xs font-medium mt-0.5 item-num-badge`;
                    item.querySelector('.item-title').className = `text-xs md:text-sm font-semibold text-slate-700 break-keep leading-snug item-title flex items-start space-x-2`;
                }}
            }});
...
print("Successfully applied 2/5 (40%) sidebar width and matched font sizes to navigation bar!")

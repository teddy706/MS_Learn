...
# Update Slide 04 with larger minimum fonts
slide_04_body = """
        ...
        <div class="col-span-12 lg:col-span-4 space-y-4">
            ...
            <div class="p-4 bg-slate-50 rounded-2xl border border-slate-200">
                ...
                <div class="grid grid-cols-2 gap-2.5 p-2.5 bg-white rounded-xl shadow-xs border border-slate-200">
                    <div class="flex items-center space-x-2 p-2 bg-sky-50 rounded-lg text-sm font-bold text-sky-800"><span>🌊</span> Edge</div>
                    <div class="flex items-center space-x-2 p-2 bg-blue-50 rounded-lg text-sm font-bold text-blue-800"><span>📝</span> Word</div>
                    <div class="flex items-center space-x-2 p-2 bg-indigo-50 rounded-lg text-sm font-bold text-indigo-800"><span>💬</span> Teams</div>
                    <div class="flex items-center space-x-2 p-2 bg-emerald-50 rounded-lg text-sm font-bold text-emerald-800"><span>📊</span> Excel</div>
                    <div class="flex items-center space-x-2 p-2 bg-blue-50 rounded-lg text-sm font-bold text-blue-900"><span>✉️</span> Outlook</div>
                    <div class="flex items-center space-x-2 p-2 bg-purple-50 rounded-lg text-sm font-bold text-purple-800"><span>➕</span> More</div>
                </div>
            </div>
        </div>
            ...
            <div class="space-y-4">
                    ...
                    <div class="p-4 bg-slate-50 rounded-xl border border-slate-200">
                        ...
                        <div class="grid grid-cols-2 gap-3 text-sm">
                            <div class="p-3 bg-white rounded-xl border border-slate-200 flex items-center space-x-3 shadow-2xs">
                                <span class="w-8 h-8 rounded-lg bg-sky-500 text-white flex items-center justify-center text-sm font-bold">E</span>
                                <div>
                                    <div class="font-bold text-slate-800 text-sm">Exchange</div>
                                    <div class="text-xs text-slate-500">mailboxes</div>
                                </div>
                            </div>
                            <div class="p-3 bg-white rounded-xl border border-slate-200 flex items-center space-x-3 shadow-2xs">
                                <span class="w-8 h-8 rounded-lg bg-blue-500 text-white flex items-center justify-center text-sm font-bold">☁️</span>
                                <div>
                            ...
                            <div class="p-3 bg-white rounded-xl border border-slate-200 flex items-center space-x-3 shadow-2xs">
                                <span class="w-8 h-8 rounded-lg bg-indigo-600 text-white flex items-center justify-center text-sm font-bold">T</span>
                                <div>
                                    <div class="font-bold text-slate-800 text-sm">Teams</div>
                                    <div class="text-xs text-slate-500">chat & channels</div>
                                </div>
                            </div>
                            <div class="p-3 bg-white rounded-xl border border-slate-200 flex items-center space-x-3 shadow-2xs">
                                <span class="w-8 h-8 rounded-lg bg-teal-600 text-white flex items-center justify-center text-sm font-bold">S</span>
                                <div>
                                    <div class="font-bold text-slate-800 text-sm">SharePoint</div>
                                    <div class="text-xs text-slate-500">files, lists, docs</div>
                                </div>
                            </div>
                        </div>
                    </div>
                ...
                <div class="grid grid-cols-2 gap-4">
                    <div class="p-4 bg-gradient-to-r from-blue-600 via-purple-600 to-pink-500 text-white rounded-2xl shadow-md flex items-center space-x-3">
                        <div class="w-10 h-10 rounded-xl bg-white/20 flex items-center justify-center text-xl shadow-xs">✨</div>
                        <div>
...
for part_idx, part in enumerate(modules):
    for slide_idx, slide in enumerate(part["slides"]):
        ...
        # Replace tiny fonts across all bodies
...
upscaled_web_html = f"""<!DOCTYPE html>
...
<body class="h-full flex flex-col antialiased">
    ...
    <header class="no-print h-14 bg-white/95 backdrop-blur-md border-b border-slate-200 text-slate-800 flex items-center justify-between px-4 z-40 shrink-0 shadow-xs">
        <div class="flex items-center space-x-3">
            ...
            <div class="flex items-center space-x-2.5">
                <span class="w-7 h-7 rounded-lg copilot-gradient-badge text-white flex items-center justify-center text-sm shadow-xs">✨</span>
                <span class="font-black text-sm md:text-base tracking-tight text-slate-900">Microsoft 365 Copilot</span>
                <span class="text-slate-300 text-xs">|</span>
                <span class="text-xs md:text-sm text-slate-600 font-bold hidden sm:inline uppercase tracking-wider">통신·네트워크 실무 마스터</span>
            </div>
        </div>
        ...
        <div class="hidden lg:flex items-center space-x-1.5 overflow-x-auto py-1">
            {"".join([f'''
            <button onclick="goToPart({idx})" class="part-pill-btn ms-pill-tab px-3.5 py-1 text-xs md:text-sm font-bold flex items-center space-x-1.5" data-part="{idx}">
        ...
        <div class="flex items-center space-x-2.5">
            ...
            <div class="relative hidden md:block">
                <input type="text" id="searchInput" placeholder="프롬프트 / 시나리오 검색..." class="bg-slate-100/90 border border-slate-200 rounded-full px-3.5 py-1 text-xs md:text-sm text-slate-700 placeholder-slate-400 focus:outline-none focus:border-indigo-500 focus:bg-white w-48 transition-all">
            </div>
            ...
            <div class="flex-1 overflow-y-auto p-2.5 space-y-1.5" id="slideListNav">
                ...
                <div class="slide-nav-item p-3 rounded-2xl cursor-pointer transition-all text-left flex items-start space-x-3 hover:bg-slate-50 border border-transparent" id="nav-item-{idx}" onclick="goToSlide({idx})">
                    ...
                    <div class="flex-1 min-w-0">
                        <div class="text-sm font-bold text-slate-800 truncate leading-snug item-title">{s["title"]}</div>
        ...
        <main class="flex-1 flex flex-col overflow-hidden relative">
            ...
            <div id="appThemeHeader" class="no-print h-11 bg-white/80 backdrop-blur-sm border-b border-slate-200 text-slate-700 px-6 flex items-center justify-between transition-all duration-500 shrink-0">
                <div class="flex items-center space-x-2.5 text-xs md:text-sm font-semibold">
                    <span id="bannerAppBadge" class="px-3 py-0.5 bg-slate-100 text-slate-800 rounded-full font-bold uppercase tracking-wider text-xs border border-slate-200">M365 COPILOT</span>
            ...
            <!-- Single Card View Container (Fluid Web Layout with Generous Font Hierarchy) -->
                ...
                <article id="portal-slide-{idx}" class="max-w-5xl mx-auto ms-fluid-card p-8 md:p-12 slide-page">
                    <div>
                        <div class="flex items-center justify-between mb-4">
                            <span class="px-3.5 py-1 rounded-full text-xs md:text-sm font-black uppercase tracking-wider bg-slate-100 text-slate-800 border border-slate-200">{s["app_name"]} • {s["badge"]}</span>
    ...
    <script>
        ...
        function renderSlide(index) {{
            ...
            // Render Center Card in Fluid Web Mode with High-Legibility Typography
            ...
            card.innerHTML = `
                <div>
                    ...
                    <div class="flex items-center justify-between mb-4">
                        <div class="flex items-center space-x-2.5">
                            <span class="w-8 h-8 rounded-lg copilot-gradient-badge text-white flex items-center justify-center text-sm shadow-xs">${{slide.app_icon}}</span>
                            <span class="px-3.5 py-1 rounded-full text-xs md:text-sm font-black uppercase tracking-wider bg-slate-100 text-slate-800 border border-slate-200">
            ...
            document.querySelectorAll('.slide-nav-item').forEach((item, i) => {{
                if (i === index) {{
                    ...
                    item.querySelector('.item-title').className = `text-sm font-black text-slate-900 truncate leading-snug item-title`;
                    ...
                    item.querySelector('.item-title').className = `text-sm font-bold text-slate-700 truncate leading-snug item-title`;
                }}
            }});
...
with open(output_path, "w", encoding="utf-8") as f:
    f.write(upscaled_web_html)

with open(index_path, "w", encoding="utf-8") as f:
    f.write(upscaled_web_html)

print(f"Successfully upscaled minimum font sizes at {output_path} and {index_path}")
